      ...
      mergeMap((project) => {
        return this.http
          ...
          .pipe(
            take(1),
            concatMap((task) =>
              combineLatest([of(task), this.tasks$]).pipe(take(1)),
            ),
            tap(([newTask, currenTasks]) => {
              newTask.taskAssignee = task.taskAssignee;
              newTask.taskLeader = task.taskLeader;
              const syncfusionFormat: ISyncfusionFormat<IAppTask> = {
                result: [...currenTasks.result, newTask],
                count: currenTasks.count + 1,
              };
      ...
      mergeMap((project) => {
        return this.http
          ...
          .pipe(
            ...
            tap((currentTasks) => {
              const updatedTasks = currentTasks.result.map((t) =>
                t.id === task.id ? task : t,
              );
              const syncfusionFormat: ISyncfusionFormat<IAppTask> = {
                result: updatedTasks,
