...
export class NotificationPanelComponent {
  ...
  constructor(private presenceService:PresenceService) {
    ...
    this.unreadNotificationsLength$ = presenceService.unreadNotificationsIds$.pipe(map(x=>{ return x.length}));

    this.allNotificationsCount$ = presenceService.allNotificationsCount$;

    this.notifications$=this.presenceService.notifications$;
    this.curentNotificationsCount$=this.presenceService.notifications$.pipe(map(x=>{ return x.length}));
  }
